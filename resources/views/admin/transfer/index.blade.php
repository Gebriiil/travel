...
@section('content')
    ...
    <div class="page-bar">
        ...
        <ul class="page-breadcrumb">
            ...
            <li>
                <a href="{{ route('admin.get.transfer.index') }}" class="active-bread">Transfers</a>
                <i class="fa fa-circle"></i>
            </li>
            <li>
    ...
    <div class="note note-info">
        <h3> @lang('site.view') ( @lang('site.transfers') ) </h3>
    </div>
        ...
        <div class="col-md-12">
            ...
            <div class="portlet light bordered">
                ...
                <div class="portlet-body">
                    ...
                    @if($transfers->count())
                        <form action="{{ route('admin.post.transfer.deleteMulti') }}" method="post"
                              id="Form2"> @csrf </form>
                        ...
                        <table
                            ...
                            @foreach($transfers as $cat)
                                <tr class="odd gradeX draggable-item">
                                    ...
                                    <td class="text-center"> {{ $cat->from_place }} </td>
                                    <td class="text-center"> {{ $cat->to_place }} </td>
                                    <td class="text-center"> {{ $cat->arrival_date }} </td>
                                    <td class="text-center"> {{ $cat->to_date }} </td>
                                    <td class="text-center"> {{ $cat->car_type }} </td>
                                    <td class="text-center"> {{ $cat->adult_number }} </td>
                                    <td class="text-center"> {{ $cat->children_number }} </td>
                                    <td class="text-center"> {{ $cat->have_infants }} </td>
                                    <td class="text-center"> {{ $cat->name }} </td>
                                    <td class="text-center"> {{ $cat->email }} </td>
                                    <td class="text-center"> {{ $cat->phone }} </td>
                                    <td class="text-center"> {{ $cat->notes }} </td>
                                    <td class="text-center">
                                        <div class="btn-group">
                                            <a href="{{ route('admin.get.transfer.delete',$cat->id) }}"
